...
servidor.post("/categorias", async (request, reply) => {
  ...
  await sql.query("INSERT INTO categorias (nome, descricao) VALUES ($1, $2)", [
    nome,
    descricao,
  ]);
...
servidor.put("/categorias/:id", async (request, reply) => {
  ...
  const busca = await sql.query("SELECT * FROM categorias WHERE id = $1", [id]);
...
servidor.delete("/categorias/:id", async (request, reply) => {
  ...
  await sql.query("DELETE FROM categorias WHERE id = $1", [id]);
...
servidor.post("/produtos", async (request, reply) => {
    ...
    if (nome === undefined || preco === undefined || quantidade === undefined || categoria === undefined) {
        return reply
        .status(400)
        .send({ error: "Nome, preço, quantidade e categoria são obrigatórios!" });
    }
    await sql.query(
...
servidor.put("/produtos/:id", async (request, reply) => {
    ...
    if (!nome || preco === undefined || quantidade === undefined || categoria === undefined) {
        return reply.status(400).send({
            error: "Os campos nome, preço, quantidade e categoria são obrigatórios!",
        });
    }
